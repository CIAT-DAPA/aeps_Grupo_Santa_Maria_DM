...
labores$Cantidad_porc[labores$Cantidad_porc>100] <- 100
...
labores$HasLote[labores$Labor %in% "FERTILIZACION FOLIAR( BOMBA MOTOR)"] <- NA
...
fertilizacion <-
ddply(labores,~Finca+Lote+Anio+Labor_resumida,summarise,conteo_labor=length(Labor_resumida),
...
embolse <- read.csv('DATOS/Embolse/Embolse_Cinta-semana30.csv',stringsAsFactors = F)
...
embolse <- embolse[c('finca', 'anio','Semana', 'Lote', 'Embolse')]
...
write.csv(consolidado3,'DATOS/consolidado_todas_variables_embolse.csv',row.names = F)
...
replaceAnio <- function(event,wks=52){
...
consolidado_embolse$Embolse[consolidado_embolse$ID %in% to_replace_NA] <- NA

write.csv(consolidado_embolse,'DATOS/consolidado_embolse_removiendo_interv.csv') # base de datos para clustering
...
colidad_fina_clima <-
  do.call(rbind,lapply(1:nrow(consolidado_todas_variables),function(w){
    ...
    dates_embolse <- data.frame(Date = as.Date((date_embolse-173):date_embolse,origin = "1970-01-01")) # En total 173 dias se tienen en cuenta un dato sacado de la literatura
